...
import { loginSchema, verifyTokenSchema, changePasswordSchema, formatZodError } from './lib/validator.js';
import { findUserByUsername, verifyPassword, createToken, validateToken, revokeUserTokens, extractTokenFromHeader, updateUserPassword } from './lib/auth.js';
...
async function handleLogin(req, res) {
    try {
        ...
        if (!rateCheck.allowed) {
            return sendError(res, req, `Muitas tentativas de login. Tente novamente em ${rateCheck.remaining_minutes} minuto(s).`, 429);
        }
        // Buscar usuário
...
async function handleLogout(req, res) {
    try {
        ...
        await logSecurityEvent('Erro no logout', 'ERROR', {
