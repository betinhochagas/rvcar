...
export const phoneSchema = z.string().regex(/^[0-9]{10,11}$/, 'Telefone deve ter 10 ou 11 dígitos');
// Validação de CPF/CNPJ
export const documentSchema = z.string().regex(/^[0-9]{11}$|^[0-9]{14}$/, 'Documento deve ter 11 (CPF) ou 14 (CNPJ) dígitos');
// Validação de ano
...
export const booleanSchema = z.boolean().or(z.enum(['true', 'false', '1', '0']).transform((val) => val === 'true' || val === '1'));
/**
 * Schema de Login
 */
 ...
 */
export function sanitizeHtml(input, allowBasicTags = false) {
    if (allowBasicTags) {
