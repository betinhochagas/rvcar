...
async function handlePost(req, res) {
    try {
        ...
        if (!token) {
            await logSecurityEvent('Tentativa de upload sem autenticação', 'WARNING', { ip: req.headers['x-forwarded-for'] || 'unknown' });
            return sendError(res, req, 'Autenticação obrigatória para upload', 401);
        }
        const user = await validateToken(token);
        if (!user) {
            await logSecurityEvent('Tentativa de upload com token inválido', 'WARNING', { ip: req.headers['x-forwarded-for'] || 'unknown' });
